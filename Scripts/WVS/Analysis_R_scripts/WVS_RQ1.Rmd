...
for (llm in unique(df$LLM)) {
  ...
  stats <- chisq.test(table(
    factor(filter(df, LLM == llm)$human_nation, levels = c("China", "USA")),
    factor(filter(df, LLM == llm)$cosine > median(filter(df, LLM == llm)$cosine), 
           levels = c(TRUE, FALSE))
  ))
  
  cat(llm, ": X = ", stats$statistic, ", p = ", stats$p.value, sep = "", "\n")
...
summary(lmer(
  cosine ~ 
    ...
    (Chinese_ppt + 1|LLM) + (Chinese_origin + 1|participant), 
  df))
...
summary(lm(
  cosine ~ Chinese_ppt * Chinese_origin, 
  df
  ))
